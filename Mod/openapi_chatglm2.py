...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    if len(prev_messages) > 0 and prev_messages[0].role == "system":
        query = (
            prev_messages.pop(0).content + query
...
if __name__ == "__main__":
    ...
    model_path = "D:/text-generation-webui/models/chatglm2"
    embeddings_model_path = "D:/yutuber_ai/m3e-base"

    model, tokenizer = auto_mode.auto_load_model(model_path)
    ...
    if is_loder_lora:
        ...
        for adapter in adapter_path:
            model = auto_mode.load_adapter(model, adapter)

    embeddings_model = SentenceTransformer(embeddings_model_path, device="cpu")

    model.eval()

    uvicorn.run(app, host="10.203.15.9", port=8000, workers=1)
