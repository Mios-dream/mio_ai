...
parser.add_argument(
    "-c", "--cut", type=int, default=5, help="default: 5 按标点符号切分"
)
parser.add_argument(
    "-fp",
    "--full_precision",
...
def get_tts_wav(
    *,
    ...
    text += "."
    print(
        f"{refer_wav_path=},{prompt_text=},{prompt_language=},{text=},{text_language=}"
    )
    ...
    text = text.strip("\n")
    for t in splits:
        text = text if not re.search(rf"\{t}", text) else re.sub(rf"\{t}+", t, text)
    ...
    audio_opt = []
    if not ref_free:
        phones1, word2ph1, norm_text1 = get_cleaned_text_final(
            prompt_text, prompt_language
        )
        bert1 = get_bert_final(
            phones1, word2ph1, norm_text1, prompt_language, device
        ).to(dtype)
